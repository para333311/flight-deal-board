...
### 실제 쿼리문을 알아내는 순서 (배포 후)

`GET /api/flights/schema?pw=1111&telegram=1` 하나로 아래를 차례로 시도한다.

1. **introspection** — 스키마에 필드·타입 이름을 직접 묻는다. 단 네이버는
   운영 환경에서 이를 꺼놨다(`__schema` 요청이 거부됨).
2. **페이지 내장 JSON** — 확인 결과 없음. HTML은 껍데기만 오고 운임은
   이후 XHR로 불러온다 (state 변수가 `__OTEL_*` 뿐).
3. **JS 번들 스캔** (`discover_queries`) — introspection이 막혀 있을 때 남은
   자동 경로. 브라우저가 보내는 쿼리문은 번들에 문자열로 실려 오므로,
   `<script src>`를 받아 `operationName`과 쿼리 정의를 추출한다.

셋 다 실패하면 브라우저 F12 → Network에서 실제 요청을 캡처하는 수밖에 없다.
