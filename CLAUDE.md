...
- `flight_search.py` — 항공권 검색. 앱 설정에 의존하지 않게 짜여 있어
  ...
  - **개발 환경에서는 네이버로 나갈 수 없다** (egress 차단). 그래서 요청
    레시피는 실물 검증이 안 된 상태이며, 실제 응답은 배포 후
    `/api/flights/probe`로만 확인할 수 있다.
  - 수집 실패와 '조건에 맞는 특가 없음'을 반드시 구분할 것. 뭉뚱그리면
    네이버가 막혔을 때 "특가 없음"이라는 거짓 알림이 나간다.
